...
const TotalAnswers = ({ data }: { data: IAnswer[] }) => {
    ...
    const COLOR_STYLES: { [i: number]: string } = {
        1: 'border-l-8 border-[#238fee] border-l-[#238fee] bg-[#212D57] ',
        2: 'border-l-8 border-[#5E91A4] border-l-[#5E91A4] bg-[#1D354D]',
        3: 'border-l-8 border-[#a28cf8] border-l-[#a28cf8] bg-[#33115B]',
        4: 'border-l-8 border-[#8B405F] border-l-[#8B405F] bg-[#431F39]',
        5: 'border-l-8 border-[#f1e2bb] border-l-[#f1e2bb] bg-[#413B25]',
    }

    return (
        <div className="justify-center px-5 flex flex-col gap-5 bg-[#212741] rounded-lg shadow-xl w-auto max-lg:p-2 max-lg:flex-row flex-wrap">
            ...
            {data.map((item, index) => (
                <div key={index} className={`border rounded-lg ${COLOR_STYLES[index + 1]} flex items-center p-7 gap-4 max-md:w-full max-lg:max-w-[250px] max-h-[150px]`}>
